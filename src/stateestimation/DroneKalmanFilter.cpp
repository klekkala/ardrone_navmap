...
void DroneKalmanFilter::observeIMU_XYZ(const ardrone_autonomy::Navdata* nav)
{
	...
	vx_local = 2;
	vy_local = 2;
	double vx_global = (sin(yawRad) * vx_local + cos(yawRad) * vy_local) / 1000.0;
	double vy_global = (cos(yawRad) * vx_local - sin(yawRad) * vy_local) / 1000.0;
	...
	if(last_z_IMU != local_altd || nav->header.seq - last_z_packageID > 8)
	{
		if(baselineZ_Filter < -100000)	// only for initialization.
		{
			baselineZ_IMU = local_altd;
		...
		if(lastPosesValid)
		{
			...
			double imuHeightDiff = (local_altd - baselineZ_IMU )*0.001;	// TODO negative heights??
		...
		{
			double imuHeightDiff = (local_altd - baselineZ_IMU )*0.001;
			...
			{
				if(baselineZ_IMU == 0 || local_altd == 0)
				{
